...
export default async function DashboardPage() {
  ...
  // ⚠️ CRITICAL: Fetch ALL fields including borrower_email
  const { data: transactions, error: transactionError } = await supabase
    .from("transactions")
    .select("id, user_id, type, amount, description, date, created_at, borrower_email, reminder_count, last_reminder_sent_at")
    .eq("user_id", user.id)
